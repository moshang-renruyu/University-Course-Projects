...
public class SijiController {
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        params.put("sijiDeleteStart",1);params.put("sijiDeleteEnd",1);
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            SijiEntity sijiEntity = new SijiEntity();
//                            sijiEntity.setSijiUuidNumber(data.get(0));                    //工号 要改的
//                            sijiEntity.setSijiName(data.get(0));                    //司机姓名 要改的
//                            sijiEntity.setSijiPhone(data.get(0));                    //手机号 要改的
//                            sijiEntity.setSijiNumber(data.get(0));                    //司机工龄 要改的
//                            sijiEntity.setSijiEmail(data.get(0));                    //电子邮箱 要改的
//                            sijiEntity.setSexTypes(Integer.valueOf(data.get(0)));   //性别 要改的
//                            sijiEntity.setSijiDelete(1);//逻辑删除字段
//                            sijiEntity.setCreateTime(date);//时间
                        ...
                        //查询是否重复
                        //工号
                        List<SijiEntity> sijiEntities_sijiUuidNumber = sijiService.selectList(new EntityWrapper<SijiEntity>().in("siji_uuid_number", seachFields.get("sijiUuidNumber")).eq("siji_delete", 1));
                        ...
                        //手机号
                        List<SijiEntity> sijiEntities_sijiPhone = sijiService.selectList(new EntityWrapper<SijiEntity>().in("siji_phone", seachFields.get("sijiPhone")).eq("siji_delete", 1));
